...
;;mocked fns
#_(defn euclidean-structure [selectors f]
  (let [erg nil #_(self/euclidean-structure selectors f)]
    (println "klm eulstru script" selectors f)
    erg))
...
(defn differential-of
  ...
  (loop [dx dx]
    (if (instance? Differential dx)
      (recur (coefficient (last (.-terms dx)))) ;klm
      dx)))
  ...
  (letfn [(extract
          ...
          (dist
            ...
            (cond (struct/structure? obj) (struct/mapr dist obj)
                  ...
                  (o/operator? obj) (do (throw (js/Error. "can't differentiate an operator yet"))
...
(defn ^:private differential->terms
  ...
  [dx]
  (cond (instance? Differential dx) (let [^Differential d dx] (.-terms d)) ;;klm
        (v/nullity? dx) []
        :else [[empty-tags dx]]))
...
(defn euclidean-structure ;;klm ^:private removed
  ...
  (letfn [(structural-derivative [g v]
            (cond (struct/structure? v)
                  ...
                  :else
                  (throw (js/Error. (str "bad structure " g ", " v)))))
          (a-euclidean-derivative [v]
            (cond (struct/structure? v)
                  ...
                  :else
                  (throw (js/Error. (str "Bad selectors " f selectors v)))))]
      ...
      [:between 1 2] (make-df (fn [x & y]
                                  (if (nil? y)
                                    ...
                                    ((d (fn [[x y]] (f x y)))
                                     (matrix/seq-> (cons x y))))))
      (fn [& xs]
        (when (empty? xs) (throw (js/Error. "No args passed to derivative?")))
        (if (= (count xs) 1)
          ((d f) (first xs))
          ((d #(apply f %)) (matrix/seq-> xs)))))))

(defmethod g/partial-derivative
  [:sicmutils-cljs.function-a/function cljs.core/PersistentVector]
  [f selectors]
  (println "klm partial der :funktion" f)
  (multivariate-derivative f selectors))

(comment
  (isa? :sicmutils-cljs.value/function :sicmutils-cljs.function-a/function) ;;true
  (isa? :sicmutils-cljs.function-a/function :sicmutils-cljs.value/function) ;;false
  (isa? (type []) ISequential) ;;false unfortunetely
  (satisfies?  ISequential []) ;;true
  (isa? (type []) cljs.core/PersistentVector) ;;true
  #_end)
