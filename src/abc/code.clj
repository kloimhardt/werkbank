...
;;(print-text ">" "")
;;----------------------------------------------------------------------
(mute-print "id" [(let [colors [midnight midnight midnight midnight yellow yellow white white]] (draw (style {:opacity 0.9} (gen-rect (val-cyc frame colors) 0 0 "100vw" "100%")))) (when-not (nth-frame 8 frame) (gen-line-grid midnight 4 80 80 {:col 20, :row 20})) (->> (gen-circ white (* 0.5 (clojure.core/deref width)) (* 0.5 (clojure.core/deref height)) (val-cyc frame (vct [100 100 100 100 200 200 200 200] [200 50 50 50 50 50 200 200 200 200] [200 200 200 200 100 100 100 100]))) (draw) (when (nth-frame 4 frame))) (->> (gen-shape (pattern (:id navy-lines)) oct) (style {:transform (str "translate(70vw, 10vh) scale(" (val-cyc frame (vct [.5 .5 .5 .5 .5 .5] [1 1 1 1 1 1] [.9 .9 .9 .9 .9 .9])) ")")}) (draw) (when (nth-frame 4 frame))) (lahatom klmbb2 (->> (gen-shape mint oct) (style {:transform "translate(10vw, 30vh) scale(2) rotate(45deg)"}) (style {:mix-blend-mode "luminosity"}) (anim "woosh" "4s" "infinite") (draw))) (lahatom klmbb4 (->> (gen-shape yellow oct) (style {:transform "translate(10vw, 30vh) scale(2) rotate(45deg)"}) (style {:mix-blend-mode "color-dodge"}) (anim "woosh" "2s" "infinite") (draw))) (when (nth-frame 7 frame) (freak-out (clojure.core/deref width) (clojure.core/deref height) 4 1000 gray)) (when (nth-frame 6 frame) (scatter klmb 10 (->> (gen-circ navy 10 10 60) (draw)))) (when (nth-frame 8 frame) (scatter klmd 10 (->> (gen-circ yellow 10 10 60) (style {:mix-blend-mode "screen"}) (draw)))) (when (nth-frame 6 frame) (scatter klme 10 (->> (gen-circ yellow 10 10 60) (style {:mix-blend-mode "overlay"}) (draw)))) (when (nth-frame 6 frame) (scatter klmf 10 (->> (gen-circ pink 10 10 60) (style {:mix-blend-mode "multiply"}) (draw))))])
(la-habra [(when (nth-frame 6 frame) (scatter klmf 10 (->> (gen-circ pink 10 10 60) (style {:mix-blend-mode "multiply"}) (draw))))])
