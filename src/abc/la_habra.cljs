...
(def levels1 (map-indexed (fn [idx color] (->> (gen-rect color -100 -100 "120%" "120%" (url "cutout")) (style {:opacity 0.4, :transform-origin "center", :transform (str (str "translate(" (- (rand-int 200) 100) "px, " (- (rand-int 300) 100) "px)") (str "rotate(" (- 360 (rand-int 720)) "deg)"))}) (anim "fade-in-out" "10s" "infinite" {:delay (str (* .1 idx) "s")}) (draw) (atom))) (take 10 (repeatedly (fn [] (nth [mint navy navy mint] (rand-int 6)))))))
(defn cx [frame] (list (do "blinking screen yellow black gery" (let [colors [midnight midnight midnight midnight yellow yellow white white]] (draw (style {:opacity 0.9} (gen-rect (val-cyc frame colors) 0 0 "100vw" "100%"))))) (do "black grid" (when-not (nth-frame 8 frame) (gen-line-grid midnight 4 80 80 {:col 20, :row 20}))) (do "blinking grey circle" (->> (gen-circ white (* 0.5 (clojure.core/deref width)) (* 0.5 (clojure.core/deref height)) (val-cyc frame (vct [100 100 100 100 200 200 200 200] [200 50 50 50 50 50 200 200 200 200] [200 200 200 200 100 100 100 100]))) (draw) (when (nth-frame 4 frame)))) (do "grey oct" (->> (gen-shape (pattern (:id navy-lines)) oct) (style {:transform (str "translate(70vw, 10vh) scale(" (val-cyc frame (vct [.5 .5 .5 .5 .5 .5] [1 1 1 1 1 1] [.9 .9 .9 .9 .9 .9])) ")")}) (draw) (when (nth-frame 4 frame)))) (do "moving green oct" (deref klmbb2)) (do "fast moving yellow oct" (deref klmbb4)) (do "lots of points" (when (nth-frame 7 frame) (freak-out (clojure.core/deref width) (clojure.core/deref height) 4 1000 gray))) (do "blue circles" (when (nth-frame 6 frame) (deref klmb))) (do "yellow circles -screen-" (when (nth-frame 8 frame) (deref klmd))) (do "yellow circles -overlay-" (when (nth-frame 6 frame) (deref klme))) (do "red circles" (when (nth-frame 6 frame) (deref klmf))) (do "grey dotted circle" (->> (gen-circ (pattern (:id white-dots)) (* 0.5 (clojure.core/deref width)) (* 0.5 (clojure.core/deref height)) 200) (draw) (when (nth-frame 1 frame)))) (do "red shape fullscreen" (->> (gen-shape pink b2) (style {:transform "translate(20vw, 30vh) scale(2)"}) (draw))) (do "reddish art" (deref klmbb)) (do "blueish art" (doall (map deref levels1)))))
