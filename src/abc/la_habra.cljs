...
(defn cx [frame] (list (do "blinking screen yellow black gery" (let [colors [midnight midnight midnight midnight yellow yellow white white]] (draw (style {:opacity 0.9} (gen-rect (val-cyc frame colors) 0 0 "100vw" "100%"))))) (comment "black grid" (when-not (nth-frame 8 frame) (gen-line-grid midnight 4 80 80 {:col 20, :row 20}))) (comment "blinking grey circle" (->> (gen-circ white (* 0.5 (clojure.core/deref width)) (* 0.5 (clojure.core/deref height)) (val-cyc frame (vct [100 100 100 100 200 200 200 200] [200 50 50 50 50 50 200 200 200 200] [200 200 200 200 100 100 100 100]))) (draw) (when (nth-frame 4 frame)))) (comment "grey oct" (->> (gen-shape (pattern (:id navy-lines)) oct) (style {:transform (str "translate(70vw, 10vh) scale(" (val-cyc frame (vct [.5 .5 .5 .5 .5 .5] [1 1 1 1 1 1] [.9 .9 .9 .9 .9 .9])) ")")}) (draw) (when (nth-frame 4 frame)))) (comment "moving green oct" (deref klmbb2)) (comment "fast moving yellow oct" (deref klmbb4)) (comment "lots of points" (when (nth-frame 7 frame) (freak-out (clojure.core/deref width) (clojure.core/deref height) 4 1000 gray))) (comment "blue circles" (when (nth-frame 6 frame) (deref klmb))) (comment "yellow circles -screen-" (when (nth-frame 8 frame) (deref klmd))) (comment "yellow circles -overlay-" (when (nth-frame 6 frame) (deref klme))) (comment "red circles" (when (nth-frame 6 frame) (deref klmf))) (comment "grey dotted circle" (->> (gen-circ (pattern (:id white-dots)) (* 0.5 (clojure.core/deref width)) (* 0.5 (clojure.core/deref height)) 200) (draw) (when (nth-frame 1 frame)))) (comment "red shape fullscreen" (->> (gen-shape pink b2) (style {:transform "translate(20vw, 30vh) scale(2)"}) (draw))) (comment "reddish art" (deref klmbb)) (comment "blueish art" (doall (map deref levels1)))))
